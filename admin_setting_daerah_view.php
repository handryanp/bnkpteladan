...
$query_sektor = "SELECT sektor FROM trsektor WHERE id = $id_sektor";
$result_sektor = $conn->query($query_sektor);

if ($result_sektor->num_rows > 0) {
    $row_sektor = $result_sektor->fetch_assoc();
    $sektor = htmlspecialchars($row_sektor['sektor']);
} 

$query = "SELECT l.*
          FROM trlingkungan l
          INNER JOIN trsektor_trlingkungan st ON st.trlingkungan_id = l.id
          WHERE st.trsektor_id = $id_sektor";
...
<div class="sidebar sidebar-bunker">
    ...
    <section style="position: relative; bottom: 140px; background-color: white;" class="home-section">
        ...
        <div class="row g-5">
            <h2 class="m-0 text-secondary" style="position: relative;">Setting Lingkungan untuk Sektor <?= $sektor ?></h2>
            <?php
            ...
            <?php endif; ?>
            <a href="admin_lingkungan_create.php"><i class="hvr-buzz-out fas fa-plus-circle" style="font-size: 35px; color:#5D5A88; position: relative;" title="Tambah Lingkungan"></i></a>
                          ...
                          <table class="table display table-bordered table-striped table-hover basic dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 132.133px;" aria-label="Position: activate to sort column ascending">Nama Lingkungan</th>
                            ...
                            <tbody>
                                                    <?php
                                                        ...
                                                        $sql = "SELECT * FROM `bnkpteladan`.`trlingkungan`";
                                                        ...
                                                        if ($result->num_rows > 0) {
                                                            while($row = $result->fetch_assoc()) {
                                                                echo "<tr role='row'>";
                                                                echo "<td>" . $no++ . "</td>";
                                                                echo "<td>" . $row["lingkungan"] . "</td>";
                                                                echo "<td>";
                                                                echo "<a href='admin_lingkungan_delete.php?id=" . $row["id"] . "' onclick='return confirm(\"Apakah Anda yakin ingin Hapus Lingkungan ini?\");' title='Hapus'><i class='fas fa-trash' style='font-size: 20px; color: red;'></i></a>";
                                                                echo "</td>";
                                                                echo "</tr>";
                                                            }
                                                        }
                                                        else {
                                                            echo "<tr><td colspan='5'>Tidak ada data lingkungan nih!</td></tr>";
                                                        }
                                                        $conn->close();
                                                        ?>
                                                    </tbody>
